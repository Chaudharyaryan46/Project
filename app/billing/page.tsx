...
export default function BillingDashboard() {
   ...
   const fetchStats = async () => {
      try {
         const res = await fetch('/api/billing/revenue?hotelId=cmnrp3c210000133y8972ejla');
   ...
   const handleOpenShift = async () => {
      ...
      try {
         const res = await fetch('/api/billing/shift', {
            ...
            body: JSON.stringify({ hotelId: 'cmnrp3c210000133y8972ejla' })
         });
         if (res.ok) {
            await fetchStats();
         }
            ...
            <div className="grid gap-4">
               {activeTables.map((table) => (
                  <motion.div
                     ...
                     key={table.id}
                     className={`p-5 premium-card transition-all cursor-pointer flex items-center justify-between ${selectedTable === table.id
                           ? 'border-2 border-webbill-burgundy ring-4 ring-webbill-burgundy/5'
                           : ''
                        }`}
                  >
                     <div className="flex items-center gap-4">
                        <div className={`w-14 h-14 rounded-2xl flex flex-col items-center justify-center font-black ${table.status === 'WAITING_BILL' ? 'bg-webbill-burgundy text-white' : 'bg-webbill-cream text-webbill-burgundy'
                           }`}>
                     ...
                     <div className="text-right">
                        ...
                        <span className={`text-[10px] font-black uppercase tracking-widest ${table.status === 'WAITING_BILL' ? 'text-red-500 animate-pulse' : 'text-blue-500'
                           }`}>
                           ...
                           {stats.recentTransactions?.length === 0 ? (
                              ...
                              stats.recentTransactions.map((tx: any) => (
                                 <tr key={tx.id} className="hover:bg-webbill-cream/30 transition-colors group">
                                    ...
                                    <td className="p-5">
                                       <span className={`px-2 py-1 rounded-md text-[9px] font-black uppercase tracking-wider ${tx.paymentMethod === 'Cash' ? 'bg-green-50 text-green-600' :
                     ...
                     <div className="grid grid-cols-3 gap-3 mb-8">
                        {[
                           ...
                           <button
                              ...
                              className={`flex flex-col items-center justify-center gap-2 p-4 rounded-2xl border-2 transition-all ${selectedPaymentMethod === method.name
                              ...
                              try {
                                 ...
                                 const res = await fetch('/api/billing/settle', {
                                    ...
                                    body: JSON.stringify({
                                       ...
                                       hotelId: 'cmnrp3c210000133y8972ejla',
                        ...
                        <div className="flex-1 space-y-2">
                           {[
                              ...
                              <button
                                 ...
                                 className={`w-full flex items-center gap-4 p-4 rounded-2xl font-black text-xs uppercase tracking-widest transition-all ${activeTab === tab.id
                                       ...
                                       onClick={async () => {
                                          ...
                                          try {
                                             const closeRes = await fetch('/api/billing/shift/close', {
                                                method: 'POST',
                                                headers: { 'Content-Type': 'application/json' },
                                                body: JSON.stringify({ hotelId: 'cmnrp3c210000133y8972ejla', closedBy: 'Counter Admin' })
                                             });
                                             ...
                                             doc.setFontSize(10);
                                             doc.setFont('helvetica', 'normal');
                                             doc.text(`Hotel: Saffron Bay (cmnrp3c210000133y8972ejla)`, 20, 35);
