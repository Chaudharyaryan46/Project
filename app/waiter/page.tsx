...
export default function WaiterPanel() {
  ...
  const fetchTables = async () => {
    try {
      const res = await fetch('/api/tables?hotelId=cmnrp3c210000133y8972ejla');
  ...
  const fetchAllOrders = async () => {
    try {
      const res = await fetch(`/api/orders?hotelId=cmnrp3c210000133y8972ejla`);
      const data = await res.json();
  ...
  const fetchMenu = async () => {
    try {
      const res = await fetch(`/api/menu?hotelId=cmnrp3c210000133y8972ejla`);
      const data = await res.json();
      setMenu(data);
  ...
  useEffect(() => {
    if (selectedTableId) {
      const active = allOrders.filter((o: any) => o.tableId === selectedTableId);
  ...
  const handlePassOrder = async () => {
    try {
      ...
      await placeOrder(selectedTableId, 'cmnrp3c210000133y8972ejla');
          ...
          {tables.map((table) => {
            ...
            return (
              <motion.button
                whileTap={{ scale: 0.95 }}
                key={table.id}
                onClick={() => { setSelectedTableId(table.id); setSelectedTableNumber(table.number); setCurrentView('MENU'); }}
                className={`bg-white p-5 rounded-[28px] border-2 relative flex flex-col items-start transition-all shadow-sm ${status === 'billed' ? 'border-red-500 bg-red-50/20' :
                  status === 'active' ? 'border-orange-400 bg-orange-50/20' :
                    'border-gray-100'
                  }`}
              >
                <div className="flex justify-between w-full items-start mb-4">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black ${status === 'billed' ? 'bg-red-500 text-white' :
                    status === 'active' ? 'bg-orange-400 text-white' :
                      'bg-webbill-cream text-webbill-burgundy'
                    }`}>
                    {table.number}
                  </div>
                  <div className={`px-2 py-1 rounded-lg text-[8px] font-black uppercase tracking-tighter flex items-center gap-1 ${status === 'billed' ? 'bg-red-100 text-red-600' :
                    status === 'active' ? 'bg-orange-100 text-orange-600' :
                      'bg-green-100 text-green-600'
                    }`}>
                    <div className={`w-1 h-1 rounded-full ${status === 'billed' ? 'bg-red-500 animate-pulse' :
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="px-6 pt-6 pb-4">
        <div className="flex justify-between items-center mb-8 mt-4">
          ...
          <h3 className="font-bold text-webbill-dark text-lg">Table - {selectedTableNumber || '?'}</h3>
          <div className="flex gap-2">
                  ...
                  {activeTableOrders.map((order: any) => (
                    order.items.map((item: any, idx: number) => (
                      <div key={`${order.id}-${idx}`} className="flex justify-between items-center">
                        ...
                        <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-md ${order.status === 'READY' ? 'bg-green-100 text-green-600' : 'bg-webbill-tan/20 text-webbill-tan'
                          }`}>
                          {order.status}
                        </span>
                      </div>
        ...
        <div className="flex gap-4 overflow-x-auto px-6 pb-4 no-scrollbar mb-4">
          {menu.map((cat) => (
            <button
              ...
              className={`px-5 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all ${activeCategory === cat.name
